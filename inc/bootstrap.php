 ...
 *
 * This is the ONE place a theme's identity lives. Change the `ligature_` function
 * prefix and the LIGATURE_* constants and the entire theme re-prefixes, because
 ...
 * them honest.
 *
 * Why a `ligature_` function prefix and not a namespace (reversed 2026-07-20):
 * this line originally read `namespace Ligature;`, on the reasoning that callbacks
 ...
 * rewrites `ligature_` → `<slug>_`, `LIGATURE_` → `<SLUG>_`, and `Ligature_` →
 ...
 * stays a string LITERAL ('ligature'), never the LIGATURE_SLUG constant. `wp i18n
 ...
 */
define( 'LIGATURE_VERSION', '1.6165.0948' );

/**
 * Absolute filesystem path to the theme root (no trailing slash).
 */
define( 'LIGATURE_DIR', get_template_directory() );

/**
 * Public URL to the theme root (no trailing slash).
 */
define( 'LIGATURE_URI', get_template_directory_uri() );
