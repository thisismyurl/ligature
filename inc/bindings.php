 ...
 * names derive from LIGATURE_SLUG, so a theme installed beside its siblings never
 ...
 */
function ligature_register_bindings(): void {
	if ( ! function_exists( 'register_block_bindings_source' ) ) {
		return;
	}

	register_block_bindings_source(
		LIGATURE_SLUG . '/copyright',
		array(
			'label'              => esc_html__( 'Copyright line', 'ligature' ),
			'get_value_callback' => 'ligature_get_copyright_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		LIGATURE_SLUG . '/site-strings',
		array(
			'label'              => esc_html__( 'Theme strings', 'ligature' ),
			'get_value_callback' => 'ligature_get_site_string_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		LIGATURE_SLUG . '/publication-date',
		array(
			'label'              => esc_html__( 'Publication date', 'ligature' ),
			'get_value_callback' => 'ligature_get_publication_date_value',
			'uses_context'       => array(),
		)
	);

	register_block_bindings_source(
		LIGATURE_SLUG . '/footer-credit',
		array(
			'label'              => esc_html__( 'Footer credit', 'ligature' ),
			'get_value_callback' => 'ligature_get_footer_credit_value',
			'uses_context'       => array(),
		)
	);
}
add_action( 'init', 'ligature_register_bindings' );
 ...
 */
function ligature_get_site_string_value( array $source_args ): string {
	switch ( $source_args['key'] ?? '' ) {
		case 'breaking-label':
			return esc_html__( 'Breaking', 'ligature' );
		case 'home-label':
			return esc_html__( 'Back to the front page', 'ligature' );
		case 'home-url':
			return esc_url( home_url( '/' ) );
		case 'not-found-title':
			return esc_html__( 'Page not found', 'ligature' );
		case 'posts-title':
			$posts_page = (int) get_option( 'page_for_posts' );
			return $posts_page ? esc_html( get_the_title( $posts_page ) ) : esc_html__( 'Latest news', 'ligature' );
	}
	return '';
}
 ...
 */
function ligature_get_copyright_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( LIGATURE_SLUG . '/copyright_date_format', 'Y' );
	$year   = (string) current_time( $format );

	$copyright = sprintf(
		/* translators: 1: four-digit year, 2: site title. */
		esc_html__( '© %1$s %2$s. All rights reserved.', 'ligature' ),
		$year,
	 ...
	 */
	$copyright = (string) apply_filters( LIGATURE_SLUG . '/copyright_text', $copyright );
 ...
 */
function ligature_get_footer_credit_value(): string {
	$theme = wp_get_theme();
	$name  = $theme->get( 'Name' );
	...
	$text = sprintf(
		...
		esc_html__( 'Built with %s.', 'ligature' ),
		$credit
	);
	 ...
	 */
	$text = (string) apply_filters( LIGATURE_SLUG . '/footer_credit_text', $text ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
 ...
 * flow gracefully".
 *
 * A ligature date is not interactive; it is the paper's dateline. Rendering it
 ...
 */
function ligature_get_publication_date_value(): string {
	/**
	 ...
	 */
	$format = (string) apply_filters( LIGATURE_SLUG . '/publication_date_format', (string) get_option( 'date_format' ) ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
