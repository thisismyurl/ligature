 ...
 *
 * Portable: every handle derives from the LIGATURE_SLUG constant, so a re-skin needs no
 ...
 *
 * Cache-buster is each file's modification time, not the LIGATURE_VERSION constant, so a
 * CSS edit busts caches immediately during iteration without a version bump.
 */
function ligature_enqueue_assets(): void {
...
add_action( 'wp_enqueue_scripts', 'ligature_enqueue_assets' );
 ...
 * an off-origin fetch.
 */
function ligature_preload_fonts(): void {
	/**
	 ...
	 * @since 1.0.0
	 */
	$fonts = (array) apply_filters( LIGATURE_SLUG . '/preload_fonts', array() );
	...
	foreach ( $fonts as $font ) {
		...
		printf(
			...
			esc_url( LIGATURE_URI . '/' . ltrim( $font, '/' ) )
		);
	}
}
add_action( 'wp_head', 'ligature_preload_fonts', 1 );
