 ...
 */
const WELCOME_FLAG = 'ligature_welcome_notice'; // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-post action and nonce action string for dismissing the welcome notice.
 */
const DISMISS_ACTION = 'ligature_dismiss_welcome'; // hardcoded slug — CLI rewrites on re-skin.

/**
 * Admin-menu slug for the Get-started page.
 */
const GET_STARTED_SLUG = 'ligature-get-started';
...
function register_get_started_page(): void {
	add_theme_page(
		...
		sprintf( esc_html__( '%s: Get started', 'ligature' ), get_theme_name() ),
		get_theme_name(),
...
function get_started_content(): array {
	...
	$default = array(
		'lead'       => sprintf(
			/* translators: %s: theme name. */
			__( '%s is a free, full-site-editing theme built to get out of the way of your content. Here is how to make it yours.', 'ligature' ),
			$theme
		),
		'steps'      => array(
			array(
				'title' => __( 'Choose what people land on.', 'ligature' ),
				'body'  => __( "A visitor's first second decides whether they stay. Set a static front page under Settings → Reading, and give your posts a page of their own.", 'ligature' ),
			),
			array(
				'title' => __( 'Give people a way around.', 'ligature' ),
				'body'  => __( 'A site without a menu is a room without doors. Open the Site Editor, edit the header, and assign your menu to Primary Navigation.', 'ligature' ),
			),
			array(
				'title' => __( 'Start from a pattern, not a blank page.', 'ligature' ),
				'body'  => __( 'In any page or post, open the block inserter, choose Patterns, and find this theme\'s group. Drop one in and change the words.', 'ligature' ),
			),
			array(
				'title' => __( 'Make it sound like you.', 'ligature' ),
				'body'  => __( 'In the Site Editor, open Styles to change colours and typefaces. Nothing you do there can break the theme — experiment freely.', 'ligature' ),
			),
		),
		'optimize'   => array(
			__( "This theme is already fast by design: zero front-end JavaScript, self-hosted fonts that don't phone home, and tuning against the Core Web Vitals search engines actually measure.", 'ligature' ),
			__( "It's built to support WCAG 2.2 AA — real focus outlines, a skip link, sensible heading order, and motion that respects a reduce-motion setting. Keep your own copy and images to that bar and the whole site stays welcoming.", 'ligature' ),
		),
		'credit'     => __( "There's a small credit in your footer. It's a thank-you, not a tax — remove it in two clicks in the Site Editor → Footer, or filter it out in code. No hard feelings either way.", 'ligature' ),
		'developers' => array(
...
function render_get_started_page(): void {
	...
	<div class="wrap ligature-get-started">
		...
		<?php if ( ! empty( $content['credit'] ) ) : ?>
			<h2><?php esc_html_e( 'The footer credit', 'ligature' ); ?></h2>
